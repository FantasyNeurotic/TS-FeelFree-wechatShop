...
class UserService extends Service {
  ...
  async topic() {
    const cond = {
      author_user: this.ctx.user._id || this.ctx.params._id,
    };
  ...
  async detail(userId) {
    const user = await this.ctx.model.User.findOne({ _id: this.ctx.user._id || userId }).lean();
    return _.pick(user, [ '_id', 'nickname', 'email', 'avatar', 'declaration', 'labels', 'websites', 'registerTime', 'lastLoginTime' ]);
  }
  async basicInformation(userId) {
    const user = await this.ctx.model.User.findOne({ _id: userId }).lean();
    return _.pick(user, [ '_id', 'nickname', 'email', 'avatar', 'declaration', 'labels', 'websites', 'registerTime', 'lastLoginTime' ]);
  }
}
